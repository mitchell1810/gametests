package tests.actions.logout.concurrent;
...
import static helpers.RestHelper.sendPostLogoutWithoutStatusCheck;
...
public class TestLogout extends BaseRestAssuredTest {
    ...
    @Story(value = "POST запрос для LOGOUT (без LOGIN) возвращает 400 ошибку и сообщение " +
            "при передаче некорректного token(incorrect token), не существующего в БД")
    public void testLogoutWithUnknownIncorrectTokenError() {

        Response response = sendPostLogoutWithoutStatusCheck(ENDPOINT, "incorrect token");
    ...
    @Epic(value = "Проверка действия пользователя")
    @Feature(value = "LOGOUT")
    @Story(value = "POST запрос для ACTION (без LOGOUT) возвращает 400 ошибку и сообщение " +
            "при передаче token, не существующего в БД и соответствующего выражению из ТЗ: \"^[0-9A-Z]{32}$\" ")
    public void testLogoutWithUnknownAlphaNumericTokenError() {

        Response response = sendPostLogoutWithoutStatusCheck(ENDPOINT, TokenGenerator.getAlphaNumericToken());
    ...
    public void testLogoutWithUnknownNullTokenError() {

        Response response = sendPostLogoutWithoutStatusCheck(ENDPOINT, null);
    ...
    @Epic(value = "Проверка действия пользователя")
    @Feature(value = "LOGOUT")
    @Story(value = "POST запрос для LOGOUT (без LOGIN) возвращает 403 ошибку и сообщение " +
            "при передаче token, не существующего в БД " +
            "и соответствующего выражению из ошибки приложения: \"^[0-9A-F]{32}$\"")
    public void testLogoutWithUnknownHexadecimalTokenError() {
    ...
    @Epic(value = "Проверка действия пользователя")
    @Feature(value = "LOGOUT")
    @Story(value = "POST запрос для LOGOUT (без LOGIN) возвращает 401 ошибку и сообщение " +
            "при передаче некорректного X-Api-Key")
    public void testLogoutInvalidXApiKeyError() {
    ...
    public void testLogoutMissedXApiKeyError() {
